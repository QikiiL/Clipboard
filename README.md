...
- **排除规则**:按来源进程(密码管理器默认拦截)、自定义正则、内置敏感识别(私钥 / 各类令牌 / 信用卡号 / 身份证号)跳过入库,命中的内容不落盘;误判时提示条上点「仍要记录」即可补录并加入豁免名单,设置里可随时清空
...
- 暂不支持多条内容批量粘贴
- 排除规则的「来源进程」在复制方进程已退出时无法判定(此时该条不生效,另两类规则照常),这是所有剪贴板管理器的共同限制
