...
**功能**:`services/sms_code_service.rs` 轮询 Windows 通知中心(`UserNotificationListener`),过滤出 Phone Link(`Microsoft.YourPhone` AUMID 前缀)转发的短信通知,提取验证码后写入剪贴板。安卓/iPhone 同一路径,手机侧零安装,数据全程本地。设计源自对 TeleLink(MIT)源码的逆向调研。

**安全设计**:

- **最小化数据面**:虽然通知监听权限技术上可读到**全部**应用的通知,但代码在拿到集合后第一步就按 AUMID 前缀过滤,非 Phone Link 的通知不做任何解析(连文本元素都不读),用后即弃、不落盘、不打日志。
- **短信全文不持久化**:通知正文只在内存中过一遍 `extract_code()`;落盘的只有验证码本身——而且是以「写剪贴板」的路径落盘,走的是与用户手动复制完全相同的本地存储(见 R-1),没有新增存储面。
- **权限前置**:功能默认关闭。开启需用户先在 设置 → 隐私和安全性 → 通知 中显式授予本应用通知访问权;`sms_code_status` 实时回报权限状态,失权时轮询退避并在设置面板标红引导。
- **提取宁缺毋滥**:`extract_code()` 要求「验证码/校验码/code 等关键词」与「4-8 位数字」同时满足且距离足够近;负向词表(尾号/卡号/账号等)排除卡号尾号;11 位手机号与 13-19 位卡号按长度排除。提取错误的代价是覆盖用户正在复制的剪贴板内容,拿不准就返回 `None`。
- **失败不 panic**:权限被撤、系统服务拒绝、剪贴板被占用(与 500ms 监控线程并发)均有重试/退避路径;监听器创建失败(老系统无该 API)时线程静默退出,不影响其余功能。

**新增残余风险**:

- 验证码写入剪贴板后,与普通复制内容一样明文进入 `clipboard.db`(R-1 的自然延伸,不新增等级)。验证码时效通常 5-10 分钟,历史清理策略照常覆盖。
- 通知监听是一个**高敏感权限**(可读全部通知)。本应用按 AUMID 过滤是代码层自律,系统层面无强制;用户应理解授予该权限的含义——设置面板的关闭开关与系统设置里的权限开关都可随时撤回。
