...
**问题**:应用把复制过的**全部**内容落盘,包括密码、信用卡号、API 密钥。用户从密码管理器复制密码、或复制一段含密钥的配置,这些内容随即明文进入 SQLite。
...
内置识别的模式:**信用卡号**(Luhn 校验,不做纯正则匹配以控制误报)、**PEM 私钥**、**AWS AKIA/ASIA**、**GitHub 令牌**(`gh[pousr]_` 与 `github_pat_`)、**Slack 令牌**、**sk- 类密钥**、**JWT**、**Bearer 令牌**、**Google API Key**(`AIza`)、**中国大陆身份证号**(校验位验证)。

**误报控制是这里的重点**,两处刻意的取舍:
- 信用卡号除正则外必须通过 **Luhn 校验**,身份证号必须通过对 **校验位** 验证——否则订单号、时间戳等普通长数字会被大量误判。
- **刻意不**实现 `password: xxx` 这类"敏感关键词 + 值"的通用匹配:代码与配置文件里 password 字样出现频率远高于真实密码泄漏,关键词匹配会制造大量误报,最终让人干脆关掉整个功能。

**健壮性**:
- 用户正则编译有 `size_limit` 上限(1MB),防巨型 NFA 拖垮应用;语法错误仅跳过该条并记日志,**绝不 panic**——panic 会让 500ms 轮询线程整个死掉。
- 内置正则用 `LazyLock` 一次编译,不在每轮轮询重建。
- `source_process_name()` 全路径返回 `None` 兜底(owner 为 NULL、OpenProcess 失败等),不 panic。

**已知限制**:
- `GetClipboardOwner()` 在**复制方进程已退出**时返回 NULL,此时无法判定来源,进程黑名单不生效(另两类规则照常)。这是所有剪贴板管理器的共同限制。
- 排除时仍会写入 `last_hash` 以避免每 500ms 重跑正则。副作用:此后删除该规则、再复制**同一份**内容不会被记录(需先复制点别的冲掉)。极边缘场景,已记入代码注释。
- 设置面板改动经 `save_settings` 立即 `reload()` 生效,无需重启。

**可见性**:命中时后端 emit `item-excluded` 事件,状态栏显示 4 秒的「已排除」提示。这条是必要的——纯静默丢弃会让规则配错变成无法察觉的数据丢失。
...
> **2026-08-29 更新**:本项已通过 F-4 排除规则**部分缓解**(来源进程黑名单 + 用户正则 + 内置敏感识别)。残余部分为"未被规则覆盖的普通内容仍会明文落盘"。
